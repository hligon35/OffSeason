...
import { showCast } from '@/lib/mock/brand'

export default function TheShowPage() {
  const featuredEpisodes = getMockFeedPage({ scope: 'episodes', page: 0, pageSize: 8 }).items
    ...
    <div className="space-y-6">
      ...
      <section className="rounded border border-brand-gray-200 bg-brand-white p-5 sm:p-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr,420px]">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-[800] tracking-tightish">What The Off Season is</h2>
      ...
      <section className="space-y-4" id="cast">
        <div className="rounded border border-brand-gray-200 bg-brand-white p-5 sm:p-6">
          ...
          <h2 className="mt-1 text-2xl font-[800] tracking-tightish">Personalities powering the show</h2>
          <p className="mt-3 text-sm text-brand-gray-700 sm:text-base">
            The show is built on voices — hosts, analysts, producers, and the roundtable that keeps the room honest.
          </p>

          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {showCast.map((p) => (
              <HostCard key={p.name} name={p.name} role={p.role} bio="Placeholder bio — personality profile modules can be expanded later." />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="episodes">
        <EpisodeList title="Featured episodes" items={featuredEpisodes} viewAllHref="#episodes" />
      </section>
